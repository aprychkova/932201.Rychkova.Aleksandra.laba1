...
int main() {
    ...
    while (1) {
        FD_ZERO(&fds);
        FD_SET(server_socket, &fds); // Добавление серверного сокета
        ...
        // Вычисляние максимального файлового дескриптора
        ...
        if (activity == -1) {
            if (errno == EINTR) {
                // Если сигнал был получен, обработываем его
                if (wasSigHup) {
        ...
        // Проверка для нового соединения 
